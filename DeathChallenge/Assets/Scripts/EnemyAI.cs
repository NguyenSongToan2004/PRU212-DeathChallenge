...
    [Header("Player & Patrol")]
    ...
    public GameObject[] playerPrefab; // Kéo GameObject của người chơi vào đây
...
        currentState = AIState.Patrol;
...

        {
            ...
            case 0:
                ...
                currentTarget = playerPrefab[0];

                break;

            case 1:

                currentTarget = playerPrefab[1];

                break;

            case 2:

                currentTarget = playerPrefab[2];
...
        if (currentTarget == null)
...

        {
            ...
            Debug.Log("EnemyAI: Player array is not null, finding closest player.");
...
    //    foreach (Transform p in playerPrefab)
...
        distanceToPlayer = Vector2.Distance(transform.position, currentTarget.transform.position);
...
                Vector2 directionAwayFromPlayer = (transform.position - currentTarget.transform.position).normalized;
...
        roamPosition = (Vector2)currentTarget.transform.position + Random.insideUnitCircle * roamRadius;
...
        }
...

    {
        ...
        if (bulletPrefab == null || bulletPoint == null || currentTarget == null || isPlayerDead)
